...
$result = $mysqli->query($sql);
 if ($result)  {
    echo "H βάση δεδομένων $dbname δημιουργήθηκε επιτυχώς\n";
      ...
      echo 'Σφάλμα κατά τη δημιουργία της βάσης δεδομένων: ' . $mysqli->error . "\n";
...
if ($mysqli->query($sql) === TRUE) {
    ...
    echo "<br> Σφάλμα κατά τη δημιουργία του πίνακα " . $mysqli->error;
   ...
   if (($handle = fopen ($data, "r")) !== FALSE) {
       while (($datacsv = fgetcsv($handle, 1000, ";")) !== FALSE) {
        ...
        if (count($datacsv) != $expectedColumnNum) {
            echo " <br>Το αρχείο που δοκιμάσατε να εισάγετε δεν περιέχει των αριθμό στηλών που περιέχει το <br>
            αρχείο που εξάγει το MySchool";
		echo "<br>Ελέξτε αν προσπαθείτε να μεταφορτώσετε όντως αρχείο CSV και αν ΝΑΙ ελέγξτε αν 
		είναι αυτό πού προέκυψε από τη λειτουργία εξαγωγής του MySchool";
            exit();
        }
